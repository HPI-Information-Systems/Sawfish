...
result_path = pathlib.Path(script_path / "grow_row_scaling_token.txt").resolve()
    ...
    for sample_idx in range(number_of_samples):
        ...
        for data_point in range(number_of_datapoints):
            ...
            for tok in tokenMode:
                ...
                command = ("java -Xmx4g -cp metanome-cli-1.1.1.jar:similarity_ind-1.1-SNAPSHOT.jar de.metanome.cli.App -o file:grow_row_scaling_token/result_%s_%s_%s_%s --algorithm de.metanome.algorithms.similarity_ind.AlgorithmInterface --files ../data/samples/current_row_samples.csv --input-key INPUT_FILES --separator ';' --escape \\\\ --header --algorithm-config maxMemoryUsagePercentage:70 " + options + " | tail -n 2") % (ds, data_point, sample_idx, str(tok))
                #print(command)
