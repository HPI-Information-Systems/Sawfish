        ...
        for idx, current_combination in enumerate(current_combinations_set):
            with open(sample_path, 'w', newline='') as csvfile:
                ...
                while True:
                    ...
                    writer.writerow(row)
                    index += 1
            command = "java -Xmx4g -cp metanome-cli-1.1.1.jar:similarity_ind-1.1-SNAPSHOT.jar de.metanome.cli.App -o file:column_scaling_token/result_%s_%s_%s --algorithm de.metanome.algorithms.similarity_ind.AlgorithmInterface --files ../data/samples/current_samples.csv --input-key INPUT_FILES --separator ';' --escape \\\\ --header --algorithm-config runBinderFirst:false ignoreShortStrings:false editDistanceThreshold:0 measureTime:false maxMemoryUsagePercentage:70 tokenMode:true similarityThreshold:0.4 | tail -n 2" % (ds, len(current_combination), idx)
