...
result_file = pathlib.Path(script_path / "../results/combined_stats/all_results.csv")
...
for ds in datasets:
    for algo in algorithms:
            for i in range(0, 3):
                ...
                command = (
                     f'java -Xmx4g -cp {classpath} de.metanome.cli.App '
                ...
                if "Error" not in output.stdout:
                     with open(result_file, 'a') as f:
                        ...
                        if runtime and results_count:
                             ...
                             f.write("%s,%s,%s,0,unlimited,100,100,True,competitors,%s,%s\n" % (i, simplified_algo_name, ds, runtime, results_count))
            ...
            if algo != "de.metanome.algorithms.binder.BinderFileAlgorithm":
                for i in range(0, 3):
                    ...
                    if algo != "de.metanome.algorithms.sindbaseline.SINDBaseline":
                        config= config +",maxMemoryUsagePercentage:70"
                        simplified_algo_name = "SAWFISH"
                    else:
                        simplified_algo_name = "BASELINE"
                    ...
                    if "Error" not in output.stdout:
                        with open(result_file, 'a') as f:
                            ...
                            if runtime and results_count:
                                runtime = runtime.group(1)
                                results_count = results_count.group(1)
                                f.write("%s,%s,%s,1,unlimited,100,100,True,competitors,%s,%s\n" % (i, simplified_algo_name, ds, runtime, results_count))
