...
all_results_token_csv = pathlib.Path(combined_stats_path / "results_jac.csv").resolve()
...
data = data[(data["dataset"].isin(datasets)) & (data["column_share"] == 100) & (data["row_share"] == 100)]
binder_data = pandas.read_csv(all_results_csv.resolve())
binder_data = binder_data[(binder_data["dataset"].isin(datasets)) & (binder_data["column_share"] == 100) & (binder_data["row_share"] == 100) & (binder_data["algorithm"] == "BINDER")]
...
fig.tight_layout()
plt.savefig(pathlib.Path(write_path / "competitor_jac.pdf"), bbox_extra_artists=(lgd, t1, t2), dpi=300, bbox_inches='tight')
#plt.show()
